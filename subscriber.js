...
function connectToBroker() {
  ...
  mqttClient.on("message", (topic, message, packet) => {
    ...
    if(obj.Type == clientId){
      ...
      if(obj.hasOwnProperty("currentSpeed")){
        messageTextArea.value = obj.currentSpeed;
      } else {
      if(obj.Failure){
        ONBtn.classList.remove("active");
        OffBtn.classList.add("active");
      ...
      //const tid = Date.now() - obj.Time;
      //console.log("HÄR KOMMER TIDEN DET TOG FÖR ATT PUBLISHER SKULLE SKICKA TILL SEUBSCRIBER" + (tid / 1000));
      ...
      if(!isNaN(parseInt(obj.Speed)) && ONBtn.classList.contains("active")){
        console.log("skriver ett nytt värde!")
